...
## 動作の仕組み
- GitHub Actionsが**毎日**21:00 UTC(=6:00 JST)と01:00 UTC(=10:00 JST)の
  2トリガーで自動起動する（`.github/workflows/collect.yml`）。各ジョブは
  「自分が実際に起動した時刻」から18:30 JSTまでを担当ウィンドウとする
  (固定時刻ではなく動的)。GitHub Actions側にジョブ実行時間の暗黙の上限
  (実測6時間)があり、単一ジョブでは開催終盤(15時台以降)を取りこぼす
  ことが判明したため、約4時間ずらした2トリガーで全日をつなぐ設計にしている
  (詳細経緯は`collector.py`冒頭のコメント参照)。重なった時間帯は両ジョブが
  同じ項目を独立に取得しうる(スプレッドシート上で重複行になるが、欠測より
  無害と判断し許容)
